...
const parse = (input, options = {}) => {
  ...
  const { LINE_REGEX, BLOCK_EMPTY_REGEX, BLOCK_OPEN_REGEX, BLOCK_CLOSE_REGEX, PLAIN_TEXT_REGEX, disallowNestedComments } = lang;
  ...
  while (remaining !== '') {
    ...
    if ((token = scan(PLAIN_TEXT_REGEX ? PLAIN_TEXT_REGEX : /^[a-zABD-Z0-9\t ]+/, 'text'))) {
      push(new Node(token));
      continue;
    }
